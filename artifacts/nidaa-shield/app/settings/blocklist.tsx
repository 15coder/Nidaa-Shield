...
export default function BlocklistScreen() {
  ...
  const handleAdd = async () => {
    const v = input.trim();
    if (!v) return;
    await addToBlocklist(v);
    setInput("");
  };

  const handleRemove = (d: string) => {
    Alert.alert("حذف من القائمة السوداء", `هل تريد حذف ${d}؟`, [
      { text: "إلغاء", style: "cancel" },
      { text: "حذف", style: "destructive", onPress: () => removeFromBlocklist(d) },
    ]);
  };
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <PageHeader
        ...
        subtitle="نطاقات يحجبها التطبيق دائماً"
      />
      ...
      <View style={styles.inputRow}>
        ...
        <TextInput
          ...
          returnKeyType="done"
          placeholder="مثال: ads.example.com"
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            <Ionicons name="ban-outline" size={40} color={colors.mutedForeground} />
            <Text style={[styles.emptyText, { color: colors.mutedForeground }]}>
              القائمة فارغة. أضف نطاقات لحجبها.
            </Text>
          </View>
        }
        renderItem={({ item }) => (
