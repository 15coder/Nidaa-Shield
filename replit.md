...
- **nidaa-shield** (`artifacts/nidaa-shield`) — Expo mobile app "نداء شايلد".
  ...
  - **Local Expo module pattern** (Apr 27, 2026): `nidaa-vpn` is autolinked natively via the `expo.autolinking.nativeModulesDir: "./modules"` field in `package.json` (NOT listed under `dependencies` — that caused expo-doctor to flag a duplicate native module). JS resolution is provided by a Metro `extraNodeModules` alias in `metro.config.js` and a TypeScript path mapping in `tsconfig.json`. A local `.npmrc` with `node-linker=hoisted` gives the app a flat node_modules so peer deps (e.g. `expo-constants` for `expo-router`) resolve to a single physical copy.
  ...
  - Helper scripts: `pnpm --filter @workspace/nidaa-shield run doctor` (runs `expo-doctor`), `pnpm --filter @workspace/nidaa-shield run clean` (wipes `node_modules`/`.expo`/`dist`/`android`/`ios`, reinstalls, and starts Metro with `--clear`).
